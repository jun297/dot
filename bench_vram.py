...
def main(args):
    ...
    # resolution = None
    # print("Warning: resolution is None")
    ...
    batch_video = video.repeat(15, 1, 1, 1, 1)

    print("Video shape:", video.shape)
    print(f"args.inference_mode: {args.inference_mode}")
    print(f"")
    ...
    if not osp.exists(tracks_path) or args.recompute_tracks:
        ...
        with torch.no_grad():
            ...
            pred = model(
                {"video": batch_video}, mode="tracks_at_motion_boundaries", **vars(args)
            )
        tracks = pred["tracks"][0]
        print("DOT inference Time:", time() - start_time)
        if args.save_tracks:
